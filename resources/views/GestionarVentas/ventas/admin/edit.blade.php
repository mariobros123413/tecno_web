<x-app-layout>
    ...
    <x-guest-layout>
        <form method="POST" action="{{ route('admin.ventas.update', ['venta_id' => $venta->id]) }}">
            @csrf
            @method('PATCH')
            <!-- Name -->
            <!-- New ComboBox Field -->
            <div class="mt-4">
                <x-input-label for="user_id" :value="__('Seleccionar el usuario')" />
                <div class="mt-4">
            ...
            <div class="mt-4">
                ...
                <x-text-input id="montototal" class="block mt-1 w-full" type="text" name="montototal"
                    :value="old('montototal', $venta->montototal)" />
            </div>

                    ...
                    <div class="bg-white shadow-md rounded-md p-6 mt-4">
                        ...
                        <form method="POST" action="{{ route('admin.pagos.generarCobro') }}" target="QrImage">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                                <div class="flex flex-col">
                                    <label class="px-3">Buscar Guia</label>
                                            <select id="mySelect" name="tcGuiaCodigo" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"  data-twe-select-filter="true">
                                            @foreach($guias as $guia)
                                            <option value="{{$guia}}">{{$guia->codigo}} </option>
                                             @endforeach
                                            </select>
                                            <x-text-input type="hidden" name="tcGuiaId"  />
                                            <x-text-input type="hidden" name="tcGuiaCodigo"  />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="flex flex-col">
                                    ...
                                    <x-text-input type="text"  required name="tcRazonSocial" :value="__('Proyecto Final')" class="border p-2 rounded-md "/>
                                </div>
                                <div class="flex flex-col">
                                    <label class="px-3">CI/NIT</label>
                                    <x-text-input  type="text" required name="tcCiNit" :value="old('cedula')" class="border p-2 rounded-md"  required pattern="[0-9]*" required/>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="flex flex-col">
                                    <label class="px-3">Celular</label>
                                    <x-text-input type="text"  required name="tnTelefono" :value="old('celular')" class="border p-2 rounded-md" title="Ingresa solo números" pattern="[0-9]*" required/>
                                </div>
                                <div class="flex flex-col">
                                    <label class="px-3">Correo</label>
                                    <x-text-input type="text" name="tcCorreo" :value="old('correo')" class="border p-2 rounded-md" readonly/>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- Monto total -->
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="flex flex-col">
                                ...
                                <x-text-input type="text" required name="taPedidoDetalle[0][Servicio]" :value="old('servicio')" class="border p-2 rounded-md" required/>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">

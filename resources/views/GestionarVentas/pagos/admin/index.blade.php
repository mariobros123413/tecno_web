                                ...
                                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" style="padding: 10px;">
                                    <div style="height: 100%; overflow: auto;">
                                        <table class="table table-bordered table-condensed table-striped"
                                            id="tabla-empresas" style="width: 100%; border-collapse: collapse;">
                                            ...
                                            @foreach($pagos as $pago)
                                                ...
                                                <tr>
                                                    <td style="text-align: center; border: 1px solid #ccc;">{{$pago->id}}
                                                    </td>
                                                    ...
                                                    <td style="text-align: center; border: 1px solid #ccc;">
                                                        <x-custom-button :url="'admin-pagos/edit/'"
                                                            :valor="$pago">{{ __('Editar') }}</x-custom-button>
                                                        <x-danger-button x-data=""
                                                            x-on:click.prevent="$dispatch('open-modal','{{$pago->id}}')">{{ __('Eliminar') }}</x-danger-button>
                                                        <x-modal name='{{$pago->id}}'
                                                            :show="$errors->userDeletion->isNotEmpty()" focusable>
                                                            <form method="POST"
                                                                action="{{ route('admin.pagos.destroy', ['pago_id' => $pago->id]) }}"
                                                                class="p-6">
                                                                @csrf
                                                                @method('DELETE')
                                                                <h2

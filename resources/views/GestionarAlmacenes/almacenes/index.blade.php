            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <section id="contenido_principal">
                    <div class="col-md-12" style="margin-top: 10px;">
                        ...
                        <a href="{{ route('admin.almacen.create') }}" class="btn btn-success" style="font-size: 13px; margin-top: 5px; margin-left: 5px;"> Agregar </a>
                        </div>
                    </div>
        ...
        <table class="table table-bordered table-condensed table-striped" id="tabla-empresas" style="width: 100%;">
            ...
            @foreach($almacenes as $almacen)
                ...
                <tr>
                        ...
                        <x-custom-button :url="'admin-almacen/edit/'" :valor="$almacen" >{{ __('Editar') }}</x-custom-button>
                        <x-danger-button x-data="" x-on:click.prevent="$dispatch('open-modal','{{$almacen->id}}')">{{ __('Eliminar') }}</x-danger-button>
                        <x-modal name='{{$almacen->id}}' :show="$errors->userDeletion->isNotEmpty()" focusable>
                        <form method="POST" action="{{ route('admin.almacen.destroy', ['almacen_id' => $almacen->id]) }}" class="p-6">
                       @csrf
                       @method('DELETE')

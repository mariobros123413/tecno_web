                    ...
                    <section id="contenido_principal">
                        <div class="col-md-12" style="margin-top: 10px;">
                            <div class="box box-default" style="border: 1px solid #574B90; min-height: 35px;">
                                <a href="{{ route('admin.paquete.create') }}" class="btn btn-success"
                                        ...
                                        <table class="table table-bordered table-condensed table-striped"
                                            ...
                                            @foreach($paquetes as $paquete)
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;">{{$paquete->dimensiones}}</td>
                                                    <td style="text-align: center;">{{$paquete->peso}}</td>
                                                    <td style="text-align: center;">
                                                        <x-custom-button :url="'admin-paquete/edit/'"
                                                            :valor="$paquete">{{ __('Editar') }}</x-custom-button>
                                                        <x-danger-button x-data=""
                                                            x-on:click.prevent="$dispatch('open-modal','{{$paquete->id}}')">{{ __('Eliminar') }}</x-danger-button>
                                                        <x-modal name='{{$paquete->id}}'
                                                            :show="$errors->userDeletion->isNotEmpty()" focusable>
                                                            <form method="POST"
                                                                action="{{ route('admin.paquete.destroy', ['paquete_id' => $paquete->id]) }}"
                                                                class="p-6">
                                                                @csrf
                                                                @method('DELETE')

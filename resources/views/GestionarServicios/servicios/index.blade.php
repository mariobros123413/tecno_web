                    ...
                    <section id="contenido_principal">
                        <div class="col-md-12" style="margin-top: 10px;">
                            <div class="box box-default" style="border: 1px solid #574B90; min-height: 35px;">
                                <a href="{{ route('admin.servicio.create') }}" class="btn btn-success"
                                            ...
                                            @foreach($servicios as $servicio)
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;">
                                                        <x-custom-button :url="'admin-servicio/edit/'"
                                                            :valor="$servicio">{{ __('Editar') }}</x-custom-button>
                                                        <x-danger-button x-data=""
                                                            x-on:click.prevent="$dispatch('open-modal','{{$servicio->id}}')">{{ __('Eliminar') }}</x-danger-button>
                                                        <x-modal name='{{$servicio->id}}'
                                                            :show="$errors->userDeletion->isNotEmpty()" focusable>
                                                            <form method="POST"
                                                                action="{{ route('admin.servicio.destroy', ['servicio_id' => $servicio->id]) }}"
                                                                class="p-6">
                                                                @csrf
                                                                @method('DELETE')
